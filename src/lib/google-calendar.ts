...
export async function getValidAccessToken(): Promise<{
  ...
  const baseDebug = `queryError=${connError ? connError.message : "none"} refreshTokenFp=${fp(connection?.refresh_token)} accessTokenFp=${fp(connection?.access_token)} expiresAt=${connection?.access_token_expires_at ?? "null"} connectedAt=${connection?.connected_at ?? "null"} now=${new Date().toISOString()} region=${process.env.VERCEL_REGION ?? "unknown"} deploymentId=${process.env.VERCEL_DEPLOYMENT_ID ?? "unknown"}`;
